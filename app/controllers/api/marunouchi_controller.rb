class Api::MarunouchiController < ApplicationController
  ...
  expose_with_pagination(:all) {
    Invoice.joins(:quote).where('quotes.division_id': 5).where(date: (params[:sdate].present?? params[:sdate] : Time.current.beginning_of_month)..(params[:edate].present?? params[:edate] : Time.current.beginning_of_month)).all.order(date: :asc)
  }

  expose_with_pagination(:only_card) {
    Invoice.joins(:quote).where('quotes.division_id': 5).where('quotes.subject like ?', '%名刺%').where(date: (params[:sdate].present?? params[:sdate] : Time.current.beginning_of_month)..(params[:edate].present?? params[:edate] : Time.current.beginning_of_month)).all.order(date: :asc)
  }

  expose_with_pagination(:consul) {
    Invoice.joins(:quote).where('quotes.division_id': 10).where(date: (params[:sdate].present?? params[:sdate] : Time.current.beginning_of_month)..(params[:edate].present?? params[:edate] : Time.current.beginning_of_month)).all.order(date: :asc)
  }
  ...
  def only_card
    ...
    respond_to do |format|
      format.html do
        @only_card = Invoice.joins(:quote).where('quotes.division_id': 5).where('quotes.subject like ?', '%名刺%').where(date: (params[:sdate].present?? params[:sdate] : Time.current.beginning_of_month)..(params[:edate].present?? params[:edate] : Time.current.beginning_of_month)).all.order(date: :asc)
      end
      format.csv do
