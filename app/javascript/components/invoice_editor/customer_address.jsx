...
const CustomerAddress = props => {
  ...
  const setHonorific = () => {
    ...
    setState({ ...init, honorific: !state.honorific });
  };

  const setPerson = () => {

    setState({ ...state, person: !state.person });
  };
    ...
    <Fragment>
      { state.honorific ?
        <Fragment>
          <div className='u-mt-10 c-btnMain-standard' onClick={ () => setHonorific() }>担当者ベースにする</div>
          ...
          { !state.person ?
            <div className='u-mt-10'>
              ...
              <div className='u-mt-10 c-btnMain-standard' onClick={ () => setPerson() }>担当者を無効にする</div>
            </div>
            :
            <Fragment>
              <div className='u-mt-10 c-form-label'> ※担当者名の入力が必要な場合のみ押下してください。</div>
              <div className='u-mt-10 c-btnMain-standard' onClick={ () => setPerson() }>担当者を有効にする</div>
            </Fragment>
          }
        </Fragment>
      :
