...
const DivisionSearch = props => {
  ...
  const search = search => {
    ...
    request.then(res => {
      ...
      window.alert('会社・部署情報を取得出来ませんでした。');
    });
  };
            ...
            { !state.body ?
              <div>
                <div className={ Style.DivisionSearch__form }>
                  <input type='text' className={ Style.DivisionSearch__input } placeholder='部署名で検索' onChange={ e => _onChange(e) }/>
                      ...
                      return (
                        <li { ...{key} } className={ Style.DivisionSearch__item }>
                          <h2 className={ Style.DivisinoSearch__itemName } data-number={ i }
                              onClick={ e => _onSelect(e) }>{ division.company ? division.company.name : '会社名なし' } { division ? division.division.name : '部署名なし' }</h2>
                        </li>
                      );
                    }) }
                  </ul>
