      ...
      { props.quote_projects ?
        <Fragment>
          { props.quote_projects.map((specification, index) => {
            const key = `specification-${ specification.project_id ? specification.project_id : specification.uid }-${ index }` ;
            return (
              <tr { ...{key} }>
                <td>
                  { specification.project_id ?
                    <textarea className='c-form-textarea__work-show-input__textarea2' type='textarea' defaultValue={ specification.name } onChange={ e => props.setName(index, e.target.value)}/>
                    : <InputSuggestion inputTxt={ specification.name } applyProject={ props.applyProject } setName={ props.setName } index={ index }/>
                  }
                </td>
