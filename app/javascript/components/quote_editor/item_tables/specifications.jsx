      ...
      { props.quote_projects.length > 0 ?
        <Fragment>
          { props.quote_projects.map((specification, index) => {
						const key = 'specification-' + specification.id + index ;
            return (
              <tr { ...{key} }>
              	...
              	<td><input className={ 'c-form-text' } type='text' value={ specification.unit_price } onChange={ e => props.setUnitPrice(index, e.target.value) } /></td>
