      ...
      <div className='c-flex c-flex__center c-flex-alignItems__center'>
        { props.quote.id ?
          <Fragment>
            { !props.quote.lock ?
              <div>
                <button className='c-btnMain c-btn-blue' onClick={ e => props.onSubmit(e) }>更新する</button>
              </div>
              : null
            }
            <Fragment>
                  ...
                  <Fragment>
                    { props.activity.status === 'lost' || props.activity.status === 'rejection' ?
                      ...
                      <div className='u-ml-10'>
                        <a className='c-btnMain c-btn-blue' rel='nofollow' data-method='post' href={ `/quotes/${ props.quote.id }/status?status=working` }>作業書作成</a>
                      </div>
                    }
                  </Fragment>
                  :
            ...
            <div className='u-ml-10'>
              <a className='c-btnMain' href={ `/activities?quote_id=${ props.quote.id }` }>活動履歴</a>
            </div>
            { props.task ?
              <div className='u-ml-10'>
                <a className='c-btnMain' href={ `/tasks/${ props.task.id }` }>チャット</a>
              </div>
              : null
            }
            { props.quote.drive_folder_id ?
              <div className='u-ml-10'>
                <a className='c-btnMain' target='_blank' href={ `https://drive.google.com/drive/u/0/folders/${ props.quote.drive_folder_id }` }>ドライブ</a>
              </div>
              : null
            }
          </Fragment>
