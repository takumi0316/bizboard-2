...
#  destination                :integer          default("company_name")
#
...
class Quote < ApplicationRecord
  ...
  enum reception: { acceptance: 0, mail: 10, delivery: 20, reservation: 30, bizstant: 40, reception_other: 50 }, _prefix: true

  enum deliver_type: { seat: 0, location: 10, pickup: 20, bizstant: 30, other: 40 }, _prefix: true
  ...
  def set_free_word
    ...
    self.free_word = "#{self.quote_number} #{self.subject} #{self.user&.name} #{self.division&.name} #{self.created_at} #{self.client&.company_division&.company&.name} #{self.client&.company_division&.name} #{self.client&.name} #{self.last_division} #{self.last_client}"
  end
